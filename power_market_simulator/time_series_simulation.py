...
def create_sample_network():
    ...
    创建示例网络
    """
    print("Creating sample 5-bus network for 24-hour simulation...")
    print("创建示例5节点网络用于24小时仿真...")
    
    # Create 5-bus network, simulating major cities in Guangdong
    # 创建5节点网络，模拟广东省主要城市
    network = Network(name="Guangdong24hSimulation")
    
    # Add 5 nodes representing major cities
    # 添加5个节点，代表主要城市
...
def analyze_24h_results(hourly_results):
    ...
    for hour in range(24):
        ...
        for node_id, price in hourly_results[hour].items():
            print(f"  {node_id}: {price:.2f} CNY/MWh")
    ...
    print(f"  Peak price period: {peak_hour:02d}时 ({max(system_avg_prices):.2f}元/MWh)")
